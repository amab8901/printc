...
`print!` and `eprint!` are rarely used because the user can instead choose `println!` and `eprintln!` to get the same thing but with clearer structure to more easily distinguish different outputs from each other (although there may be some use cases of `print!` and `eprint!`). Thus, `print!` and `eprint!` are suboptimal choices for producing clean & clear output with minimal boilerplate. 
